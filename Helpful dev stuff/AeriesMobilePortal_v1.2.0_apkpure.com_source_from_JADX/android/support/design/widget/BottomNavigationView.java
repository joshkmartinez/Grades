...
public class BottomNavigationView extends FrameLayout {
    ...
    class C05371 implements Callback {
        ...
        public boolean onMenuItemSelected(MenuBuilder menuBuilder, MenuItem menuItem) {
            ...
            if (BottomNavigationView.this.mReselectedListener == null || menuItem.getItemId() != BottomNavigationView.this.getSelectedItemId()) {
                if (BottomNavigationView.this.mSelectedListener == null || BottomNavigationView.this.mSelectedListener.onNavigationItemSelected(menuItem) != null) {
                    z = false;
                }
                return z;
            }
    ...
    public BottomNavigationView(Context context, AttributeSet attributeSet, int i) {
        ...
        attributeSet = TintTypedArray.obtainStyledAttributes(context, attributeSet, C0012R.styleable.BottomNavigationView, i, C0012R.style.Widget_Design_BottomNavigationView);
        if (attributeSet.hasValue(C0012R.styleable.BottomNavigationView_itemIconTint) != 0) {
            this.mMenuView.setIconTintList(attributeSet.getColorStateList(C0012R.styleable.BottomNavigationView_itemIconTint));
        } else {
            this.mMenuView.setIconTintList(createDefaultColorStateList(16842808));
        }
        if (attributeSet.hasValue(C0012R.styleable.BottomNavigationView_itemTextColor) != 0) {
            this.mMenuView.setItemTextColor(attributeSet.getColorStateList(C0012R.styleable.BottomNavigationView_itemTextColor));
        } else {
            this.mMenuView.setItemTextColor(createDefaultColorStateList(16842808));
        }
        if (attributeSet.hasValue(C0012R.styleable.BottomNavigationView_elevation) != 0) {
            ViewCompat.setElevation(this, (float) attributeSet.getDimensionPixelSize(C0012R.styleable.BottomNavigationView_elevation, 0));
        }
        this.mMenuView.setItemBackgroundRes(attributeSet.getResourceId(C0012R.styleable.BottomNavigationView_itemBackground, 0));
        if (attributeSet.hasValue(C0012R.styleable.BottomNavigationView_menu) != 0) {
            inflateMenu(attributeSet.getResourceId(C0012R.styleable.BottomNavigationView_menu, 0));
        }
        ...
        this.mMenu.setCallback(new C05371());
    }
    ...
    private void addCompatibilityTopDivider(Context context) {
        ...
        view.setBackgroundColor(ContextCompat.getColor(context, C0012R.color.design_bottom_navigation_shadow_color));
        view.setLayoutParams(new FrameLayout.LayoutParams(-1, getResources().getDimensionPixelSize(C0012R.dimen.design_bottom_navigation_shadow_height)));
        addView(view);
    }

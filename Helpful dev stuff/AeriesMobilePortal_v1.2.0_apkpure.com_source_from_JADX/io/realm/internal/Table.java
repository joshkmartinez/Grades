...
public class Table implements TableSchema, NativeObject {
    ...
    public void renameColumn(long j, String str) {
        long j2 = j;
        verifyColumnName(str);
        String nativeGetColumnName = nativeGetColumnName(this.nativePtr, j2);
        long primaryKey = getPrimaryKey();
        ...
        if (primaryKey == j2) {
            try {
                ...
                if (primaryKeyTable != null) {
                    long findFirstString = primaryKeyTable.findFirstString(0, getClassName());
                    if (findFirstString != -1) {
                        nativeSetString(primaryKeyTable.nativePtr, 1, findFirstString, str, false);
                        return;
                    }
                ...
                nativeRenameColumn(r7.nativePtr, j, nativeGetColumnName);
    ...
    public long getPrimaryKey() {
        if (this.cachedPrimaryKeyColumnIndex < 0) {
            if (this.cachedPrimaryKeyColumnIndex != -2) {
                ...
                long findFirstString = primaryKeyTable.findFirstString(0, getClassName());
                if (findFirstString != -1) {
                    this.cachedPrimaryKeyColumnIndex = getColumnIndex(primaryKeyTable.getUncheckedRow(findFirstString).getString(1));
                } else {
    ...
    private boolean isPrimaryKey(long j) {
        return (j < 0 || j != getPrimaryKey()) ? 0 : 1;
    }
    ...
    void checkDuplicatedNullForPrimaryKeyValue(long j, long j2) {
        if (isPrimaryKeyColumn(j)) {
    ...
    public static void throwDuplicatePrimaryKeyException(Object obj) {
        ...
        stringBuilder.append("Value already exists: ");
        stringBuilder.append(obj);
        throw new RealmPrimaryKeyConstraintException(stringBuilder.toString());
    }
    ...
    public String toString() {
        ...
        if (hasPrimaryKey()) {
            name = getColumnName(getPrimaryKey());
        ...
        while (true) {
            ...
            if (j < columnCount) {
                ...
                stringBuilder.append(getColumnName(j));
                i++;
            } else {
                stringBuilder.append(".");

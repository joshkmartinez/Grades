...
public final class FlowableCombineLatest<T, R> extends Flowable<R> {
    ...
    static final class CombineLatestCoordinator<T, R> extends BasicIntQueueSubscription<R> {
        ...
        void subscribe(Publisher<? extends T>[] publisherArr, int i) {
            ...
            for (int i2 = 0; i2 < i && !this.done; i2++) {
                if (this.cancelled) {
                    break;
                }
        ...
        void innerValue(int i, T t) {
            synchronized (this) {
                ...
                T t2 = this.nonEmptySources;
                if (obj[i] == null) {
                    t2++;
                    this.nonEmptySources = t2;
                }
                obj[i] = t;
                if (obj.length == t2) {
                    this.queue.offer(this.subscribers[i], obj.clone());
                    t = null;
                } else {
                    t = true;
                }
        ...
        boolean checkTerminated(boolean z, boolean z2, Subscriber<?> subscriber, SpscLinkedArrayQueue<?> spscLinkedArrayQueue) {
            ...
            if (z) {
                if (!this.delayErrors) {
                    ...
                    if (z && z != ExceptionHelper.TERMINATED) {
                        cancelAll();
                        spscLinkedArrayQueue.clear();

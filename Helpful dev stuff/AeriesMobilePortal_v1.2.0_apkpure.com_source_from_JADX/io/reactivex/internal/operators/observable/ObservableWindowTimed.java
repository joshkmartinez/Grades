...
public final class ObservableWindowTimed<T> extends AbstractObservableWithUpstream<T, Observable<T>> {
    ...
    static final class WindowExactBoundedObserver<T> extends QueueDrainObserver<T, Object, Observable<T>> implements Disposable {
        ...
        public void onSubscribe(Disposable disposable) {
            if (DisposableHelper.validate(this.f335s, disposable)) {
                this.f335s = disposable;
                disposable = this.actual;
                disposable.onSubscribe(this);
                if (!this.cancelled) {
                    UnicastSubject create = UnicastSubject.create(this.bufferSize);
                    this.window = create;
                    disposable.onNext(create);
    ...
    static final class WindowExactUnboundedObserver<T> extends QueueDrainObserver<T, Object, Observable<T>> implements Observer<T>, Disposable, Runnable {
        ...
        public void onSubscribe(Disposable disposable) {
            if (DisposableHelper.validate(this.f336s, disposable)) {
                this.f336s = disposable;
    ...
    static final class WindowSkipObserver<T> extends QueueDrainObserver<T, Object, Observable<T>> implements Disposable, Runnable {
        ...
        final class CompletionTask implements Runnable {
            private final UnicastSubject<T> f33w;

            CompletionTask(UnicastSubject<T> unicastSubject) {
                this.f33w = unicastSubject;
            }

            public void run() {
                WindowSkipObserver.this.complete(this.f33w);
            }
        }

        static final class SubjectWork<T> {
            final boolean open;
            final UnicastSubject<T> f34w;

            SubjectWork(UnicastSubject<T> unicastSubject, boolean z) {
                this.f34w = unicastSubject;
                this.open = z;
            }
        }
        ...
        public void onSubscribe(Disposable disposable) {
            if (DisposableHelper.validate(this.f337s, disposable)) {
                this.f337s = disposable;
                this.actual.onSubscribe(this);
                if (this.cancelled == null) {
                    disposable = UnicastSubject.create(this.bufferSize);
                    this.windows.add(disposable);
                    this.actual.onNext(disposable);
                    this.worker.schedule(new CompletionTask(disposable), this.timespan, this.unit);
        ...
        void drainLoop() {
            ...
            while (!this.terminated) {
                ...
                if (z && (z2 || z3)) {
                    ...
                    if (!subjectWork.open) {
                        list.remove(subjectWork.f34w);
                        subjectWork.f34w.onComplete();

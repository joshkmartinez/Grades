...
public final class CompletableOnErrorComplete extends Completable {
    ...
    final class OnError implements CompletableObserver {
        private final CompletableObserver f68s;

        OnError(CompletableObserver completableObserver) {
            this.f68s = completableObserver;
        }

        public void onComplete() {
            this.f68s.onComplete();
        }
        ...
        public void onError(Throwable th) {
            try {
                if (CompletableOnErrorComplete.this.predicate.test(th)) {
                    ...
                    this.f68s.onError(th);
                }
            } catch (Throwable th2) {
                Exceptions.throwIfFatal(th2);
                this.f68s.onError(new CompositeException(th, th2));
            }
        }

        public void onSubscribe(Disposable disposable) {
            this.f68s.onSubscribe(disposable);
        }
    }

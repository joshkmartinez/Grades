...
public final class ParallelReduceFull<T> extends Flowable<T> {
    ...
    static final class ParallelReduceFullMainSubscriber<T> extends DeferredScalarSubscription<T> {
        ...
        ParallelReduceFullMainSubscriber(Subscriber<? super T> subscriber, int i, BiFunction<T, T, T> biFunction) {
            super(subscriber);
            subscriber = new ParallelReduceFullInnerSubscriber[i];
            for (int i2 = 0; i2 < i; i2++) {
                subscriber[i2] = new ParallelReduceFullInnerSubscriber(this, biFunction);
            }
        ...
        SlotPair<T> addValue(T t) {
            while (true) {
                SlotPair<T> slotPair = (SlotPair) this.current.get();
                if (slotPair == null) {
                    slotPair = new SlotPair();
                    if (!this.current.compareAndSet(null, slotPair)) {
                    }
                }
                int tryAcquireSlot = slotPair.tryAcquireSlot();
                if (tryAcquireSlot >= 0) {
                    break;
                }
        ...
        void innerComplete(T t) {
            if (t != null) {
                while (true) {
                    ...
                    try {
                        t = ObjectHelper.requireNonNull(this.reducer.apply(t.first, t.second), "The reducer returned a null value");
                    } catch (T t2) {
                        Exceptions.throwIfFatal(t2);
                        innerError(t2);
                        return;

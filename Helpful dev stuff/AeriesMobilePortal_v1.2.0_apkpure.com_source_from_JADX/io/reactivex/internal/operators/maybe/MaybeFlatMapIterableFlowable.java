...
public final class MaybeFlatMapIterableFlowable<T, R> extends Flowable<R> {
    ...
    static final class FlatMapIterableObserver<T, R> extends BasicIntQueueSubscription<R> implements MaybeObserver<T> {
        ...
        public void onError(Throwable th) {
            this.f320d = DisposableHelper.DISPOSED;
            ...
            while (!this.cancelled) {
                try {
                    ...
                    if (!this.cancelled) {
                        ...
                        } catch (Iterator<? extends R> it2) {
                            Exceptions.throwIfFatal(it2);
                            subscriber.onError(it2);
                            return;
                        }
                    }
                    return;
                } catch (Iterator<? extends R> it22) {
                    Exceptions.throwIfFatal(it22);
                    subscriber.onError(it22);
                    return;
                }
            }
        }

...
public final class SingleDoOnEvent<T> extends Single<T> {
    ...
    final class DoOnEvent implements SingleObserver<T> {
        private final SingleObserver<? super T> f196s;

        DoOnEvent(SingleObserver<? super T> singleObserver) {
            this.f196s = singleObserver;
        }

        public void onSubscribe(Disposable disposable) {
            this.f196s.onSubscribe(disposable);
        }

        public void onSuccess(T t) {
            try {
                SingleDoOnEvent.this.onEvent.accept(t, null);
                this.f196s.onSuccess(t);
            } catch (T t2) {
                Exceptions.throwIfFatal(t2);
                this.f196s.onError(t2);
            }
        }

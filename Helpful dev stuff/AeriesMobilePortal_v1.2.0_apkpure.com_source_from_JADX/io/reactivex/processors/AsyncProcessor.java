...
public final class AsyncProcessor<T> extends FlowableProcessor<T> {
    ...
    protected void subscribeActual(Subscriber<? super T> subscriber) {
        ...
        if (!add(asyncSubscription)) {
            ...
            if (subscriber != null) {
                asyncSubscription.complete(subscriber);
            } else {
                asyncSubscription.onComplete();
            }
        } else if (asyncSubscription.isCancelled() != null) {
    ...
    boolean add(AsyncSubscription<T> asyncSubscription) {
        ...
        do {
            ...
            int length = asyncSubscriptionArr.length;
            obj = new AsyncSubscription[(length + 1)];
            System.arraycopy(asyncSubscriptionArr, 0, obj, 0, length);
            obj[length] = asyncSubscription;
        } while (!this.subscribers.compareAndSet(asyncSubscriptionArr, obj));
        return true;
    }

...
public class TestObserver<T> extends BaseTestConsumer<T, TestObserver<T>> implements Observer<T>, Disposable, MaybeObserver<T>, SingleObserver<T>, CompletableObserver {
        ...
        if (disposable == null) {
            ...
            if (this.initialFusionMode != 0 && (disposable instanceof QueueDisposable)) {
                ...
                if (requestFusion == 1) {
                    ...
                    while (true) {
                        try {
                            disposable = this.qs.poll();
                            if (disposable == null) {
                                break;
                            }
                            this.values.add(disposable);
                        } catch (Disposable disposable2) {
                            this.errors.add(disposable2);
                        }
                    }
                    this.completions++;
            ...
            this.actual.onSubscribe(disposable2);
        } else {
            disposable2.dispose();
    ...
    public final TestObserver<T> assertOf(Consumer<? super TestObserver<T>> consumer) {
        try {
            ...
            return this;
        } catch (Consumer<? super TestObserver<T>> consumer2) {

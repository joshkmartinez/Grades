...
class AdvertisingInfoProvider {
    ...
    public AdvertisingInfo getAdvertisingInfo() {
        ...
        if (isInfoValid(infoFromPreferences)) {
            Fabric.getLogger().mo2301d(Fabric.TAG, "Using AdvertisingInfo from Preference Store");
        ...
        new Thread(new BackgroundPriorityRunnable() {
            public void onRun() {
                ...
                if (!advertisingInfo.equals(access$000)) {
                    Fabric.getLogger().mo2301d(Fabric.TAG, "Asychronously getting Advertising Info and storing it to preferences");
    ...
    private AdvertisingInfo getAdvertisingInfoFromStrategies() {
        ...
        if (isInfoValid(advertisingInfo)) {
            Fabric.getLogger().mo2301d(Fabric.TAG, "Using AdvertisingInfo from Reflection Provider");
        } else {
            advertisingInfo = getServiceStrategy().getAdvertisingInfo();
            if (isInfoValid(advertisingInfo)) {
                Fabric.getLogger().mo2301d(Fabric.TAG, "Using AdvertisingInfo from Service Provider");
            } else {
                Fabric.getLogger().mo2301d(Fabric.TAG, "AdvertisingInfo not present");
            }
        }

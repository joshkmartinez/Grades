...
public class FileStoreImpl implements FileStore {
    ...
    File prepare(File file) {
        if (file != null) {
            if (!file.exists()) {
                if (!file.mkdirs()) {
                    Fabric.getLogger().mo2314w(Fabric.TAG, "Couldn't create file");
                }
            }
            return file;
        }
        Fabric.getLogger().mo2301d(Fabric.TAG, "Null File");
        return null;
    }
    ...
    boolean isExternalStorageAvailable() {
        ...
        Fabric.getLogger().mo2314w(Fabric.TAG, "External Storage is not mounted and/or writable\nHave you declared android.permission.WRITE_EXTERNAL_STORAGE in the manifest?");
        return false;
    }
}

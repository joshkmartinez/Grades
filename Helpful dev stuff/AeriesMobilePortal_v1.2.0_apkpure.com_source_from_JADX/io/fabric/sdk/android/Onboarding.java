...
class Onboarding extends Kit<Boolean> {
    ...
    protected boolean onPreExecute() {
        try {
            ...
            Fabric.getLogger().mo2304e(Fabric.TAG, "Failed init", e);
            return false;
        }
    }
    ...
    protected Boolean doInBackground() {
        ...
        if (retrieveSettingsData != null) {
            try {
                ...
                performAutoConfigure = performAutoConfigure(appIconHashOrNull, retrieveSettingsData.appData, mergeKits(map, this.providedKits).values());
            } catch (Throwable e) {
                Fabric.getLogger().mo2304e(Fabric.TAG, "Error performing auto configuration.", e);
            }
    ...
    private SettingsData retrieveSettingsData() {
        try {
            ...
            Fabric.getLogger().mo2304e(Fabric.TAG, "Error dealing with settings", e);
            return null;
        }
    }
    ...
    private boolean performAutoConfigure(String str, AppSettingsData appSettingsData, Collection<KitInfo> collection) {
        if (AppSettingsData.STATUS_NEW.equals(appSettingsData.status)) {
            if (performCreateApp(str, appSettingsData, collection) != null) {
                return Settings.getInstance().loadSettingsSkippingCache();
            }
            Fabric.getLogger().mo2304e(Fabric.TAG, "Failed to create app with Crashlytics service.", null);
            return null;
        } else if (AppSettingsData.STATUS_CONFIGURED.equals(appSettingsData.status)) {
            return Settings.getInstance().loadSettingsSkippingCache();
        } else {
            if (appSettingsData.updateRequired) {
                Fabric.getLogger().mo2301d(Fabric.TAG, "Server says an update is required - forcing a full App update.");

...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable, ByteChannel {
    ...
    class C10261 extends OutputStream {
        public void close() {
        }
    ...
    class C10272 extends InputStream {
        public void close() {
        }
    ...
    public static final class UnsafeCursor implements Closeable {
        ...
        public long resizeBuffer(long j) {
            ...
            } else if (r0.readWrite) {
                long j3 = r0.buffer.size;
                long j4;
                if (j2 <= j3) {
                    if (j2 >= 0) {
                        j4 = j3 - j2;
                        while (j4 > 0) {
                            Segment segment = r0.buffer.head.prev;
                            long j5 = (long) (segment.limit - segment.pos);
                            if (j5 > j4) {
                                segment.limit = (int) (((long) segment.limit) - j4);
                                break;
                            }
                            r0.buffer.head = segment.pop();
                    ...
                    int i = 1;
                    while (j4 > 0) {
                        Segment writableSegment = r0.buffer.writableSegment(1);
                        int min = (int) Math.min(j4, (long) (8192 - writableSegment.limit));
                        writableSegment.limit += min;
                        long j6 = j4 - ((long) min);
                        if (i != 0) {
                            r0.segment = writableSegment;
                            r0.offset = j3;
                            r0.data = writableSegment.data;
                            r0.start = writableSegment.limit - min;
                            r0.end = writableSegment.limit;
                            i = 0;
                        }
                        j4 = j6;
                    }
        ...
        public long expandBuffer(int i) {
            ...
            if (i <= 0) {
                ...
                i = this.buffer.writableSegment(i);
                int i2 = 8192 - i.limit;
                i.limit = 8192;
                long j2 = (long) i2;
                ...
                while (i4 < i5) {
                    ...
                    if (i6 >= (byte) 48 && i6 <= (byte) 57) {
                        ...
                        if (j >= -922337203685477580L) {
                            if (j != -922337203685477580L || ((long) i7) >= j2) {
                                j = (j * 10) + ((long) i7);
                            }
                        }
                        Buffer writeByte = new Buffer().writeDecimalLong(j).writeByte(i6);
                        if (i2 == 0) {
                            writeByte.readByte();
                        }
                        StringBuilder stringBuilder = new StringBuilder();
                    ...
                    } else if (i6 == Framer.STDIN_FRAME_PREFIX && i == 0) {
                        j2--;
                        i2 = 1;
                    } else if (i != 0) {
                        i3 = 1;
                        ...
                        stringBuilder2.append(Integer.toHexString(i6));
                        ...
                        if (charAt <= '?') {
                            ...
                            if (charAt <= CharCompanionObject.MAX_HIGH_SURROGATE && min >= 56320) {
                                if (min <= 57343) {
                                    ...
                                    writeByte((i6 >> 18) | DimensionsKt.HDPI);
    ...
    Segment writableSegment(int i) {
        if (i < 1 || i > 8192) {
            ...
            r1 = this.head.prev;
            if (r1.limit + i > 8192 || r1.owner == 0) {
                r1 = r1.push(SegmentPool.take());
            }
            return r1;
        }
    }

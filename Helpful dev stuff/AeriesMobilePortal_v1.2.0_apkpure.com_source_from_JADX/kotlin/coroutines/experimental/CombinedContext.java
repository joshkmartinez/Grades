...
public final class CombinedContext implements CoroutineContext {
    ...
    @Nullable
    public <E extends Element> E get(@NotNull CoroutineContext$Key<E> coroutineContext$Key) {
        Intrinsics.checkParameterIsNotNull(coroutineContext$Key, "key");
        CombinedContext combinedContext = this;
        while (true) {
            E e = combinedContext.element.get(coroutineContext$Key);
            if (e != null) {
                return e;
            }
            CoroutineContext coroutineContext = combinedContext.left;
            if (!(coroutineContext instanceof CombinedContext)) {
                return coroutineContext.get(coroutineContext$Key);
            }
    ...
    @NotNull
    public CoroutineContext minusKey(@NotNull CoroutineContext$Key<?> coroutineContext$Key) {
        Intrinsics.checkParameterIsNotNull(coroutineContext$Key, "key");
        if (this.element.get(coroutineContext$Key) != null) {
            return this.left;
        }
        coroutineContext$Key = this.left.minusKey(coroutineContext$Key);
        if (coroutineContext$Key == this.left) {
            coroutineContext$Key = this;
        } else if (coroutineContext$Key == EmptyCoroutineContext.INSTANCE) {
            coroutineContext$Key = (CoroutineContext) this.element;
        } else {
            coroutineContext$Key = new CombinedContext(coroutineContext$Key, this.element);
        }
